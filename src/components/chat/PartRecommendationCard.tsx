...
export function PartRecommendationCard({ part }: PartRecommendationCardProps) {
  ...
  return (
    <Link to={`/parts/${part.id}`} className="block">
      <Card className="w-56 hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-green-50 to-emerald-50 border-green-200">
        <div className="aspect-square relative overflow-hidden rounded-t-lg">
          ...
          <div className="absolute top-2 right-2">
            <Badge variant="secondary" className={conditionColorMap[part.condition] || ""}>
              {part.condition}
            </Badge>
          </div>
        </div>
        ...
        <CardContent className="p-3">
          ...
          <div className="text-xs text-muted-foreground mb-2">
            Seller: <span className="font-medium">{part.seller.username}</span> 
          </div>
          <div className="font-bold text-sm text-primary">{formatPrice(part.price)}</div>
        </CardContent>
      </Card>
    </Link>
  );
