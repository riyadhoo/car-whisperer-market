...
export function CarRecommendationCard({ car }: CarRecommendationCardProps) {
  ...
  return (
    <Link to={`/cars/${car.id}`} className="block">
      <Card className="w-64 hover:shadow-lg transition-shadow duration-300 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200">
        ...
        <CardContent className="p-3">
          <div className="font-semibold text-sm mb-1">{car.make} {car.model}</div>
          ...
          <div className="text-sm font-bold text-primary">{getPriceRange(car.price)}</div>
        </CardContent>
      </Card>
    </Link>
  );
